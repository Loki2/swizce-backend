...
export class AuthResolvers {
  ...
  async signUp(
    ...
    try {
      ...
      if (isUsername)
        throw new Error("Username already picked... Please try other!");
  ...
  async requestResetPassword(
    ...
    try {
      ...
      if (!response || response[0]?.statusCode !== 202)
        throw new Error("Sorry, Can not send to email...!");

      return { message: "Please Check your email for link to reset password" };
    } catch (error) {
      throw error;
    }
  }
  ...
  async resetPassword(
    ...
    try {
      ...
      //check confirm is matched password
      if(confirmpassword !== password) throw new Error("Confirm password is not matched...!")
